    ...
    with CORPUS.open(encoding="utf-8") as f:
        for line in f:
            ...
            metadata_needs_repair = (
                old_methods != methods
                or row.get("framework", {}).get("framework_id") != policy.get("framework_id")
                ...
                or row.get("content_hash") != content_hash(row.get("text", ""))
            )
            if metadata_needs_repair:
                ...
                row["content_hash"] = content_hash(row.get("text", ""))
                changed += 1
            rows.append(row)
    if changed:
